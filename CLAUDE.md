...
- **Calculation** (`calculate()`): Core formula — `qB = (qA / 100 * fA.cal) / fB.cal * 100`. Macros are scaled linearly from per-100g values. Results and macro comparison bars are rendered into `#results`.
...
- The `--green` variable is used for both interactive elements (focus rings, bars, button) and the header background — changing it affects the entire visual identity.
